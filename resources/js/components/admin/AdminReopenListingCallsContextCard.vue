...
<template>
    <div class="rounded-lg border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 px-5 py-4">
        ...
        <p v-if="formattedSubmittedAt || listing.assigned_to_name" class="mt-4 pt-3 border-t border-gray-100 dark:border-gray-800 text-xs text-gray-500 dark:text-gray-400">
            <span v-if="listing.assigned_to_name">Submitted by <span class="font-medium text-gray-700 dark:text-gray-300">{{ listing.assigned_to_name }}</span></span>
            <span v-else>Submitted</span>
            <span v-if="formattedSubmittedAt"> · {{ formattedSubmittedAt }}</span>
        </p>
    </div>
</template>
