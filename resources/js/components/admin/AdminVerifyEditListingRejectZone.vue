...
const initial            = window.__INITIAL_VERIFY_EDIT_LISTING__ ?? {};
const initialRejectErrs  = initial.rejectErrors ?? null;

const errors     = ref(initialRejectErrs ?? {});
const submitting = ref(false);

const listingUuid = initial.listing?.uuid ?? '';
...
function submit() {
    ...
    formEl.method = 'POST';
    // ?from=visited so the reject endpoint redirects back to /admin/visited-listings
    // (default reject redirect goes to /admin/unverified-listings).
    formEl.action = `/admin/listings/${listingUuid}/reject?from=visited`;
...
<template>
    <div class="mt-8 rounded-lg border border-red-200 dark:border-red-900/60 bg-red-50/40 dark:bg-red-950/20">
        ...
        <div class="px-6 py-5 flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
            <div class="flex-1">
                <h3 class="text-sm font-semibold text-gray-900 dark:text-gray-100">Reject this submission</h3>
                <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
                    Marks the listing as rejected and removes it from the verification queue. The field officer's submission stays in the audit log.
                </p>
